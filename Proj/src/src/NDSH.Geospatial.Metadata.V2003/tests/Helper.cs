...
namespace NDSH.Geospatial.Metadata.V2003.Tests {
  ...
  public class Helper {
    public static readonly Dictionary<string, string> RestrictedStringDummyValues = new(){
      {"Show", NDSH.Geospatial.Metadata.V2003.XLinks.IShow.ShowBehaviours[XLinks.ShowBehaviour.None] }, 
      {"Actuate", NDSH.Geospatial.Metadata.V2003.XLinks.IActuate.ActuateBehaviours[XLinks.ActuateBehaviour.None] }
    };
    ...
    public static object GetDummyObject(Type type) {
      ...
      if (type.IsAbstract
        || type.Name.EndsWith("_PropertyType")    // WARN: Commenting *PropertyType will cause stackoverflow. come class e..g CI_Citation has circular reference.
        || type.Name.EndsWith("PropertyType")) {  // TODO: The problem here is that soon we will need to change the names of the classes. So, this needs to change using the inheritance path and discovering if root object is AbstractObject_Type (this name will change as well).
        return null;
      }
      ...
      foreach (PropertyInfo pi in propInfos) {
        ...
        throw new NotSupportedException($"Unhandled PropertyType: " +
          $"{type.Name}.{pi.PropertyType.Name} {pi.Name} ");
      }

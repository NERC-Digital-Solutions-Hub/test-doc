...
namespace NDSH.Geospatial.Metadata.V2005.DataQuality {
  ...
  [JsonObject("DQ_Element")]
  public class DQ_Element : EntityBase<DQ_Element> {

    #region Constructors - Destructors

    /// <summary>
    /// Initialzies the <see cref="DQ_Element"/>.
    /// </summary>
    public DQ_Element() {
      _result = new List<DQ_Result>();
      _dateTime = new List<System.DateTime>();
      _evaluationProcedure = new CI_Citation();
      _evaluationMethod = new DQ_EvaluationMethodTypeCode();
    ...
    private DQ_EvaluationMethodTypeCode _evaluationMethod;
    
    /// <summary>
    /// 
    /// </summary>
    [XmlElement(Order = 3, ElementName = "evaluationMethod")]
    [JsonProperty("evaluationMethod", Order = 3)]
    public DQ_EvaluationMethodTypeCode EvaluationMethod {
      get {
        return _evaluationMethod;
      }
      set {
        if (_evaluationMethod == value) {
          return;
        }
        if (_evaluationMethod == null || _evaluationMethod.Equals(value) != true) {
          _evaluationMethod = value;
          OnPropertyChanged();
        }
      }
    }

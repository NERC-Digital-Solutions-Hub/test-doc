
#region Header
// Title Name        : 
// Member of         : NDSH.Geospatial.Metadata.V2005.dll
// Description       : 
// Created by        : 08/11/2022, 22:00, @gisvlasta.
// Updated by        : 17/12/2022, 01:32, @gisvlasta - Cleaned Xml Serialization Attributes.
// Version           : 1.0.1
// GitHub Repository : https://github.com/UoMResearchIT/nerc-digital-solutions-hub
// License           : MIT Licence
// Copyright         : 
//
// Comments          : 
#endregion

#region Imported Namespaces

using NDSH.Geospatial.Metadata.V2005.Common.Citation;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Diagnostics;
using System.Linq;
using System.Text;
using System.Threading.Tasks;
using System.Xml;
using System.Xml.Serialization;

#endregion

namespace NDSH.Geospatial.Metadata.V2005.PortrayalCatalogue {

  /// <summary>
  /// 
  /// </summary>
  [Serializable]
  [XmlRoot("MD_PortrayalCatalogueReference", Namespace = "http://www.isotc211.org/2005/gmd", IsNullable = false)]
  [XmlType("MD_PortrayalCatalogueReference", Namespace = "http://www.isotc211.org/2005/gmd")]
  [JsonObject("MD_PortrayalCatalogueReference")]
  public class MD_PortrayalCatalogueReference : Model {

    #region Constructors - Destructors

    /// <summary>
    /// Initializes the <see cref="MD_PortrayalCatalogueReference"/>.
    /// </summary>
    public MD_PortrayalCatalogueReference() {

      xmlns = new XmlSerializerNamespaces();
      xmlns.Add("gmd", "http://www.isotc211.org/2005/gmd");

      _portrayalCatalogueCitation = default; //new List<CI_Citation>();

    }

    #endregion

    #region Public Fields

    /// <summary>
    /// 
    /// </summary>
    [XmlNamespaceDeclarations()]
    public XmlSerializerNamespaces xmlns;

    #endregion

    #region Public Properties

    private List<CI_Citation> _portrayalCatalogueCitation;
    
    /// <summary>
    /// 
    /// </summary>
    [XmlElement("portrayalCatalogueCitation", Order = 0, ElementName = "portrayalCatalogueCitation")]
    [JsonProperty("portrayalCatalogueCitation", Order = 0)]
    public List<CI_Citation> PortrayalCatalogueCitation {
      get {
        return _portrayalCatalogueCitation;
      }
      set {
        if (_portrayalCatalogueCitation == value) {
          return;
        }
        if (_portrayalCatalogueCitation == null || _portrayalCatalogueCitation.Equals(value) != true) {
          _portrayalCatalogueCitation = value;
          OnPropertyChanged();
        }
      }
    }

    private string _id;
    
    /// <summary>
    /// 
    /// </summary>
    [XmlAttribute(DataType = "ID", AttributeName = "id")]
    [XmlIgnore()]
    [JsonProperty("id")]
    public string Id {
      get {
        return _id;
      }
      set {
        if (_id == value) {
          return;
        }
        if (_id == null || _id.Equals(value) != true) {
          _id = value;
          OnPropertyChanged();
        }
      }
    }

    private string _uuid;

    /// <summary>
    /// 
    /// </summary>
    [XmlAttribute(AttributeName = "uuid")]
    [XmlIgnore()]
    [JsonProperty("uuid")]
    public string Uuid {
      get {
        return _uuid;
      }
      set {
        if (_uuid == value) {
          return;
        }
        if (_uuid == null || _uuid.Equals(value) != true) {
          _uuid = value;
          OnPropertyChanged();
        }
      }
    }

    #endregion

  }

}
